...
nadpis <- "Relativní migrace" # nadpis grafu
...
tmTomioORP <- tm_shape(republika, bbox = bbox)+tm_borders("grey30", lwd = 1) +
  ...
  tm_style_white(nadpis, frame = F,  fontfamily = "Roboto", legend.text.size = 0.4, legend.title.size = 0.7, legend.format = list(text.separator = "-", fun=function(x) paste0(formatC(x, digits=4, format="f"), " %")))
