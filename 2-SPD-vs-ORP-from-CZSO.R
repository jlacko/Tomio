...
nadpis <- "Saldo migrace" # nadpis grafu
leyenda <- ""  # nadpis legendy

tmTomioORP <- tm_shape(republika, bbox = bbox)+tm_borders("grey30", lwd = 1) +
  tm_shape(wobce) + tm_fill(col = "saldo_migrace", palette = "RdYlGn", title = leyenda, textNA = "Jinak (vojenské újezdy)") +
  tm_shape(velkaMesta) + tm_borders("grey30", lwd = 0.5)+
  tm_style_white(nadpis, frame = F, legend.text.size = 0.7, legend.title.size = 1.5, legend.format = list(text.separator = "-", fun=function(x) paste0(formatC(x, digits=0, big.mark = " ", format="f"), "")))
